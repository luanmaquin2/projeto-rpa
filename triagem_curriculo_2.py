...
def enviar_email(destinatario: str, nome_candidato: str,
    ...
    if destinatario == "nao encontrado":
        print(f"      ⚠️  E-mail nao encontrado — pulando.")
        return False

    if MODO_SIMULACAO:
        print(f"[SIMULACAO] Enviaria e-mail para {destinatario}")
        return True

    try:
        options = Options()
        options.add_argument(r"--user-data-dir=C:\Users\luanm\AppData\Local\Google\Chrome\User Data")
        ...
        print("✉️ Clicando em escrever...")
        bot.find_element('//div[contains(text(),"Escrever") or contains(text(),"Compose")]', By.XPATH).click()
        bot.wait(3000)

        bot.find_element('//textarea[@name="to"]', By.XPATH).send_keys(destinatario)
        bot.find_element('//input[@name="subjectbox"]', By.XPATH).send_keys(
            f"Parabéns! Você avançou — {NOME_VAGA}"
        )

        corpo = f"""
Olá, {nome_candidato}!

Você foi aprovado na triagem inicial.
...
"""
        ...
        bot.find_element('//div[@aria-label="Corpo da mensagem" or @aria-label="Message Body"]', By.XPATH).send_keys(corpo)

        bot.wait(2000)

        print("📤 Enviando...")
        bot.find_element('//div[contains(text(),"Enviar") or contains(text(),"Send")]', By.XPATH).click()
